...
def candidate_apis_test(baseurl, header_dict, param_dict, output_dir, api_template_list, candidate_api_list,
                        api_validity_json, no_get_producer, open_isrequired, need_trigger, log_file):
    for candidate_api in candidate_api_list:
        ...
        candidate_api_producer_pool = api_template_list[:candidate_api_index] + api_template_list[
                                                                                candidate_api_index + 1:]
        ...
        for api_index in range(len(candidate_api_seq) - 1):
            ...
            if response != None:
                if api_validity_json:
                    if (api_validity_json["success_str"] in response.text) and (
                            api_validity_json["fail_str"] not in response.text):
                        if "Content-Type" in response.headers:
                            if "json" in response.headers["Content-Type"]:
                                response_json = json.loads(response.text)
                                update_api_response_value_by_response_json(current_api.api_response_value,
                                                                           response_json)
                            else:
                ...
                if candidate_api_test_type in ["proxy_api", "command_api", "display_api"]:
                    ...
                    for test_param in test_params:
                        for test_payload in test_payloads:
                            vul_location_str = (candidate_api_template.api_url + "VoAPI" + test_param).replace("{",
                                                                                                               "!").replace(
                                "}", "!")
                            ...
                            update_api_request_param_value(request_value_struct, test_param,
                                                           [test_payload, "VoAPI_TEST"])
                            test_request_dict_list = format_request(request_value_struct,
                                                                    candidate_api_template.api_request, open_isrequired)
                            response = request_sender(baseurl, candidate_api_template.api_url,
                                                      candidate_api_template.api_method, header_dict,
                                                      test_request_dict_list, log_file)
                            # sleep time for vul request
                            time.sleep(0.4)
                            if candidate_api_test_type == "display_api":
                                if (candidate_api_template.api_method.lower() == "post") and (response != None) and (
                                        str(response.status_code)[0] == "2"):
                                    record_vul_api(vul_output_dir, candidate_api_test_type, candidate_api_template,
                                                   test_param, test_payload, request_validation_api=False)
                            if os.path.exists(vul_output_dir + vul_output_file):
                                # record_vul_api
                                record_vul_api(vul_output_dir, candidate_api_test_type, candidate_api_template,
                                               test_param, test_payload, request_validation_api=True)
                            else:
                                # vul not exist
                                if need_trigger:
                                    if (candidate_api_template.api_method.lower() == "post") and (
                                            response != None) and (str(response.status_code)[0] == "2"):
                                        # need_trigger
                                        ...
                                        for api_trigger in api_triggers:
                                            ...
                                            if api_trigger.api_method.lower() != "get":
                                                ...
                                                request_value_struct["path"]["id"] = [microcks_response_json["id"],
                                                                                      "VoAPI_PRODUCER"]
                                                request_dict_list = format_request(request_value_struct,
                                                                                   api_trigger.api_request,
                                                                                   open_isrequired)
                                                request_sender(baseurl, api_trigger.api_url, api_trigger.api_method,
                                                               header_dict, request_dict_list, log_file)
                                            else:
                                                request_sender(baseurl, api_trigger.api_url, api_trigger.api_method,
                                                               header_dict, [{}, {}, {}, {}], log_file)
                                            # sleep time for vul request
                                            time.sleep(0.4)
                                            if os.path.exists(vul_output_dir + vul_output_file):
                                                record_vul_api(vul_output_dir, candidate_api_test_type,
                                                               candidate_api_template, test_param, test_payload,
                                                               request_validation_api=True)
                                                vul_flag = True
                                                break
                                #         if not vul_flag:
                    ...
                    for test_param in test_params:
                        for test_payload in test_payloads:
                            ...
                            if response != None:
                                if api_validity_json:
                                    if (api_validity_json["success_str"] in response.text) and (
                                            api_validity_json["fail_str"] not in response.text) and (
                                            ("; for 16-bit" in response.text) or ("root:" in response.text)):
                                        record_vul_api(vul_output_dir, candidate_api_test_type, candidate_api_template,
                                                       test_param, test_payload[0], request_validation_api=False)
                                    # else:
                                    #     record_undetected_suspicious_api(candidate_api_template, candidate_api_test_type, test_param, undetected_suspicious_dir)
                                else:
                                    if str(response.status_code)[0] == "2" and (
                                            ("; for 16-bit" in response.text) or ("root:" in response.text)):
                                        # record vul
                                        record_vul_api(vul_output_dir, candidate_api_test_type, candidate_api_template,
                                                       test_param, test_payload[0], request_validation_api=False)
                    ...
                    if param_dict:
                        update_api_request_param_value_by_custom_param_dict(request_value_struct, param_dict)
                    test_request_dict_list = format_request(request_value_struct, candidate_api_template.api_request,
                                                            open_isrequired)
                    try:
                        inject_param_list = sqlmap_test(baseurl, candidate_api_template.api_url,
                                                        candidate_api_template.api_method, header_dict,
                                                        test_request_dict_list, test_params, log_file)
                    except:
                        continue
                    if inject_param_list:
                        for inject_param in inject_param_list:
                            record_vul_api(vul_output_dir, candidate_api_test_type, candidate_api_template,
                                           inject_param, "SQLMap heuristic (basic) test", request_validation_api=False)
                    # else:
                    ...
                    for test_param in test_params:
                        for test_payload in test_payloads:
                            ...
                            response = request_sender(baseurl, candidate_api_template.api_url,
                                                      candidate_api_template.api_method, header_dict,
                                                      test_request_dict_list, log_file, upload_flag=True)
                            if response != None:
                                if api_validity_json:
                                    if (api_validity_json["success_str"] in response.text) and (
                                            api_validity_json["fail_str"] not in response.text):
...
def main():
    ...
    parser.add_argument('--openapi', help='OpenAPI File Path For Resolve "multipart/form-data" to Support Upload API',
                        type=str, default=None, required=False)
    parser.add_argument('--restler_compile', help='RESTler Compile File Path', type=str, default="APIInfo.txt",
                        required=False)
    parser.add_argument('--verification_server_ip', help='Verification Server IP', type=str, default="127.0.0.1",
                        required=False)
    parser.add_argument('--verification_server_port', help='Verification Server Port', type=int, default=4444,
                        required=False)
    parser.add_argument('--verification_server_port_for_https', help='Verification Server Port For Https', type=int,
                        default=4445, required=False)
    parser.add_argument('--baseurl', help='Target API Service Base Url', type=str, default="http://127.0.0.1",
                        required=False)
    parser.add_argument('--output', help='Output Dir Absolute Path', type=str, default="./", required=False)
    parser.add_argument('--upload_payloads_dir', help='Upload Payloads Dir Absolute Path', type=str, default=None,
                        required=False)
    parser.add_argument('--api_header_file', help='API Header File for Using API', type=str, default=None,
                        required=False)
    parser.add_argument('--api_content_type', help='API Request Content-Type Header for Using API', type=str,
                        default="application/json", required=False)
    parser.add_argument('--api_param_file', help='API Param File for Using API', type=str, default=None, required=False)
    parser.add_argument('--api_validity_file', help='API Validity Str File for Judging API Request Success or Not',
                        type=str, default=None, required=False)
    parser.add_argument('--api_template_file', help='API Template File for Adding API', type=str, default=None,
                        required=False)
    parser.add_argument('--no_get_producer', action="store_true", help='Get Method Can not be Producer, Default: False')
    parser.add_argument('--open_isrequired', action="store_true",
                        help='Whether Open Param isRequired Option or not, Default: False')
    parser.add_argument('--need_trigger', action="store_true",
                        help='Whether Need Trigger API to Trigger Vul, Default: False')
    ...
    adapt_api_vul_payloads(args.verification_server_ip, args.verification_server_port,
                           args.verification_server_port_for_https, args.upload_payloads_dir)
    if args.output == "./":
    ...
    if add_api_templates_list:
        for add_api_template in add_api_templates_list:
            for api_template in api_template_list:
                if (api_template.api_url == add_api_template.api_url) and (
                        api_template.api_method.lower() == add_api_template.api_method.lower()):
    ...
    if upload_apis:
        for upload_api in upload_apis:
            for api_template in api_template_list:
                if (api_template.api_url == upload_api["api_url"]) and (
                        api_template.api_method.lower() == upload_api["api_method"].lower()):
                    api_template.api_request["body"] = {
                        upload_api["multipart_param"]: ["String", [], ["MultiPartValue"], True]}
                    api_template.api_request_value["body"] = {
                        upload_api["multipart_param"]: ["MultiPartValue", "VoAPI_SPECIFICATION"]}
    ...
    if add_candidate_api_list:
        for add_candidate_api in add_candidate_api_list:
            for candidate_api in candidate_api_list:
                if (candidate_api[0].api_url == add_candidate_api[0].api_url) and (
                        candidate_api[0].api_method.lower() == add_candidate_api[0].api_method.lower()):
                    candidate_api_list.remove(candidate_api)
        candidate_api_list = add_candidate_api_list + candidate_api_list
    candidate_apis_test(baseurl, header_dict, param_dict, output_dir, api_template_list, candidate_api_list,
                        api_validity_json, args.no_get_producer, args.open_isrequired, args.need_trigger, log_file)
