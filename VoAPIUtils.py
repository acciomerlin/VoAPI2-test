                ...
                for api_method in api_methods:
                    ...
                    if multipart_param:
                        ...
                        if (path_flag and (not param_flag)) or (param_flag):
                            upload_apis.append(
                                {"api_url": api_url, "api_method": api_method, "multipart_param": multipart_param})
...
def adapt_api_vul_payloads(verification_server_ip, verification_server_port, verification_server_port_for_https,
                           upload_dir):
        ...
        if api_func in ["proxy_api", "command_api", "display_api"]:
            for i in range(len(ApiVulnerabilityPayloads[api_func])):
                if "http://" in ApiVulnerabilityPayloads[api_func][i]:
                    ApiVulnerabilityPayloads[api_func][i] = ApiVulnerabilityPayloads[api_func][i].replace("IP:PORT",
                                                                                                          verification_server_ip + ":" + str(
                                                                                                              verification_server_port))
                elif "https://" in ApiVulnerabilityPayloads[api_func][i]:
                    ApiVulnerabilityPayloads[api_func][i] = ApiVulnerabilityPayloads[api_func][i].replace("IP:PORT",
                                                                                                          verification_server_ip + ":" + str(
                                                                                                              verification_server_port_for_https))
                else:
                    pass
    if upload_dir:
            ...
            if (re.match(accurate_producer_pattern, producer_name)) and (producer_type == consumer_type):
                ...
                for temp_producer_api in temp_producer_apis:
                    if candidate_api_producer.api_url == temp_producer_api[0].api_url:
                        if ProducerMethodPriority[candidate_api_producer.api_method.upper()] < ProducerMethodPriority[
                            temp_producer_api[0].api_method.upper()]:
                            producer_flag = False
                        else:
    ...
    for consumer in extend_consumer_dict:
        ...
        if the_only_producer:
            ...
            producer_consumer_relations.append(
                {'consumer_api': candidate_api, 'consumer_param': consumer, 'producer_api': the_only_producer[0],
                 'producer_param': the_only_producer[1]})
            continue
        ...
        for extend_consumer in extend_consumer_list:
            ...
            the_only_producer = find_consumer_the_only_producer(extend_consumer, consumer_type,
        ...
        if signal_producer_apis:
            ...
            producer_consumer_relations.append(
                {'consumer_api': candidate_api, 'consumer_param': consumer, 'producer_api': the_signal_producer[0],
                 'producer_param': the_signal_producer[1]})
...
def is_valid_producer(candidate_api, candidate_api_producer, no_get_producer):
    ...
    if (candidate_api.api_url == candidate_api_producer.api_url) and (
            ProducerMethodPriority[candidate_api.api_method.upper()] > ProducerMethodPriority[
        candidate_api_producer.api_method.upper()]):
        return False
    return True
...
def format_request(request_value_struct, api_request_struct, open_isrequired=False):
    ...
    def traverse_and_format(param_dict, request_dict, param_name, api_request_part_dict, open_isrequired):
        ...
        if param_struct[0] == "Array":
            ...
            for array_param_index in range(1, len(param_struct)):
                if type(param_struct[array_param_index]) == dict:
                    ...
                    for array_param_name in param_struct[array_param_index]:
                        traverse_and_format(param_struct[array_param_index], request_struct[array_param_index],
                                            array_param_name, api_request_part_dict[param_name][-1], open_isrequired)
            ...
            for property_param_name in param_struct[1]:
                traverse_and_format(param_struct[1], request_struct[1], property_param_name,
                                    api_request_part_dict[param_name], open_isrequired)
        else:
    ...
    if request_value_struct["path"]:
        ...
        for param_name in request_value_struct["path"]:
            traverse_and_format(request_value_struct["path"], api_request_struct["path"], param_name, api_path_dict,
                                open_isrequired)
    ...
    if request_value_struct["header"]:
        ...
        for param_name in request_value_struct["header"]:
            traverse_and_format(request_value_struct["header"], api_request_struct["header"], param_name,
                                api_header_dict, open_isrequired)
    ...
    if request_value_struct["query"]:
        ...
        for param_name in request_value_struct["query"]:
            traverse_and_format(request_value_struct["query"], api_request_struct["query"], param_name, api_query_dict,
                                open_isrequired)
    ...
    if request_value_struct["body"]:
        ...
        for param_name in request_value_struct["body"]:
            ...
            if (param_name == "body") and (len(list(request_value_struct["body"].keys()))) == 1 and (
                    type(request_value_struct["body"][param_name]) == list):
                api_body_dict = []
                ...
                for array_param_index in range(1, len(param_struct)):
                    if type(param_struct[array_param_index]) == dict:
                        ...
                        for array_param_name in param_struct[array_param_index]:
                            traverse_and_format(param_struct[array_param_index], request_struct[array_param_index],
                                                array_param_name, api_body_dict[-1], open_isrequired)
                    elif type(param_struct[array_param_index]) == list:
                        if param_struct[array_param_index][1] == "VoAPI_RANDOM":
                            param_type = request_struct[array_param_index][0]
                ...
                traverse_and_format(request_value_struct["body"], api_request_struct["body"], param_name, api_body_dict,
                                    open_isrequired)
    if open_isrequired:
            ...
            for param_name in request_value_struct[request_part]:
                ...
                for custom_param in custom_param_dict:
                    if param_name == custom_param:
                        if (request_value_struct[request_part][param_name][0] == "Array") or (
                                request_value_struct[request_part][param_name][0] == "Property"):
                            request_value_struct[request_part][param_name] = [custom_param_dict[custom_param],
                                                                              "VoAPI_CUSTOM"]
                        else:
                            if ParamValuePriority[request_value_struct[request_part][param_name][1]] <= \
                                    ParamValuePriority["VoAPI_CUSTOM"]:
                                request_value_struct[request_part][param_name] = [custom_param_dict[custom_param],
                                                                                  "VoAPI_CUSTOM"]
                        custom_flag = True
                if custom_flag:
        ...
        if param_struct[0] == "Array":
                ...
                if type(param_struct[array_param_index]) == dict:
                    for array_param_name in param_struct[array_param_index]:
                        if param_struct[array_param_index][array_param_name]:
                            if (param_struct[array_param_index][array_param_name][0] != "Array") and (
                                    param_struct[array_param_index][array_param_name][0] != "Property"):
                                if (array_param_name == request_param_name):
                                    if ParamValuePriority[param_struct[array_param_index][array_param_name][1]] <= \
                                            ParamValuePriority[request_param_value_strategy]:
                                        param_struct[array_param_index][array_param_name] = request_param_value_list
                            else:
                                traverse_and_update(param_struct[array_param_index], array_param_name,
                                                    request_param_name, request_param_value_list)
                        else:
                            if (array_param_name == request_param_name):
                                param_struct[array_param_index][array_param_name] = request_param_value_list
                elif type(param_struct[array_param_index]) == list:
                    if (param_name == request_param_name):
                        if ParamValuePriority[param_struct[array_param_index][1]] <= ParamValuePriority[
                            request_param_value_strategy]:
            ...
            for property_param_name in param_struct[1]:
                if param_struct[1][property_param_name]:
                    if (param_struct[1][property_param_name][0] != "Array") and (
                            param_struct[1][property_param_name][0] != "Property"):
                        if (property_param_name == request_param_name):
                            if ParamValuePriority[param_struct[1][property_param_name][1]] <= ParamValuePriority[
                                request_param_value_strategy]:
                                param_struct[1][property_param_name] = request_param_value_list
                    else:
                        traverse_and_update(param_struct[1], property_param_name, request_param_name,
                                            request_param_value_list)
                else:
                    if (property_param_name == request_param_name):
                        param_struct[1][property_param_name] = request_param_value_list
        else:
        ...
        if request_value_struct[request_part]:
            for param_name in request_value_struct[request_part]:
                if request_value_struct[request_part][param_name]:
                    if (request_value_struct[request_part][param_name][0] != "Array") and (
                            request_value_struct[request_part][param_name][0] != "Property"):
                        if param_name == request_param_name:
                            if ParamValuePriority[request_value_struct[request_part][param_name][1]] <= \
                                    ParamValuePriority[request_param_value_strategy]:
                        ...
                        traverse_and_update(request_value_struct[request_part], param_name, request_param_name,
                                            request_param_value_list)
                else:
                    if param_name == request_param_name:
                        request_value_struct[request_part][param_name] = request_param_value_list
...
def request_sender(baseurl, api_url, api_method, header_dict, request_dict_list, log_file, upload_flag=False):
    ...
    try:
        ...
        r = requests.request(method=api_method.lower(), url=url, headers=api_header_dict, params=api_query_dict,
                             data=request_data, timeout=4)
    ...
    if type(response_json) == list:
        ...
        for param_name in response_json:
            if type(response_json[param_name]) == list:
                ...
                update_api_response_param_value(api_response_value, param_name,
                                                [response_json[param_name], "VoAPI_PRODUCER"])
    else:
                        ...
                        if param_struct[array_param_index][array_param_name]:
                            if (param_struct[array_param_index][array_param_name][0] != "Array") and (
                                ...
                                if (array_param_name == response_param_name):
                                    ...
                                    if param_struct[array_param_index][array_param_name][1] != \
                                            response_param_value_list[1]:
                                        param_struct[array_param_index][array_param_name] = response_param_value_list
                            else:
                                traverse_and_update(param_struct[array_param_index], array_param_name,
                                                    response_param_name, response_param_value_list)
                        else:
                            if (array_param_name == response_param_name):
                                param_struct[array_param_index][array_param_name] = response_param_value_list
                elif type(param_struct[array_param_index]) == list:
            ...
            for property_param_name in param_struct[1]:
                if param_struct[1][property_param_name]:
                    if (param_struct[1][property_param_name][0] != "Array") and (
                        ...
                        traverse_and_update(param_struct[1], property_param_name, response_param_name,
                                            response_param_value_list)
                else:
                    if (property_param_name == response_param_name):
                        param_struct[1][property_param_name] = response_param_value_list
        else:
        ...
        if api_response_value[api_response_part]:
            for param_name in api_response_value[api_response_part]:
                if api_response_value[api_response_part][param_name]:
                    if (api_response_value[api_response_part][param_name][0] != "Array") and (
                            api_response_value[api_response_part][param_name][0] != "Property"):
                        if (param_name == response_param_name):
                            # same name var only update by first response var
                        ...
                        traverse_and_update(api_response_value[api_response_part], param_name, response_param_name,
                                            response_param_value_list)
                else:
                    if (param_name == response_param_name):
                        api_response_value[api_response_part][param_name] = response_param_value_list
                        ...
                        for array_param_name in param_struct[array_param_index]:
                            if param_struct[array_param_index][array_param_name]:
                                if (param_struct[array_param_index][array_param_name][0] != "Array") and (
                                    ...
                                    response_param_value = traverse_and_get(param_struct[array_param_index],
                                                                            array_param_name, response_param_name)
                    elif type(param_struct[array_param_index]) == list:
                        if (param_name == response_param_name):
                ...
                for property_param_name in param_struct[1]:
                    if param_struct[1][property_param_name]:
                        if (param_struct[1][property_param_name][0] != "Array") and (
                            ...
                            response_param_value = traverse_and_get(param_struct[1], property_param_name,
                                                                    response_param_name)
            else:
                print(param_struct)
            ...
            for param_name in api_response_value[api_response_part]:
                if (api_response_value[api_response_part][param_name]):
                    if (api_response_value[api_response_part][param_name][0] != "Array") and (
                        ...
                        response_param_value = traverse_and_get(api_response_value[api_response_part], param_name,
                                                                response_param_name)
    return response_param_value
            ...
            for array_param_index in range(1, len(param_struct)):
                ...
                if type(param_struct[array_param_index]) == dict:
                    for param_name in param_struct[array_param_index]:
                        if (param_struct[array_param_index][param_name][0] != "Array") and (
                                param_struct[array_param_index][param_name][0] != "Property"):
    ...
    for api_request_part in ["header", "query", "body"]:
        if api_request[api_request_part]:
            for param_name in api_request[api_request_part]:
                temp_params = []
                if (api_request[api_request_part][param_name][0] != "Array") and (
                        api_request[api_request_part][param_name][0] != "Property"):
                    request_params.append(param_name)
                else:
                ...
                if type(param_struct[array_param_index]) == dict:
                    for param_name in param_struct[array_param_index]:
                        if (param_struct[array_param_index][param_name][0] != "Array") and (
                            ...
                            consumer_or_producer_dict.update(
                                parse_param_struct(param_struct[array_param_index][param_name]))
        elif param_struct[0] == "Property":
            for param_name in param_struct[1]:
                if (param_struct[1][param_name][0] != "Array") and (param_struct[1][param_name][0] != "Property"):
    ...
    for api_request_or_response_part in api_request_or_api_response:
        if api_request_or_api_response[api_request_or_response_part]:
            for param_name in api_request_or_api_response[api_request_or_response_part]:
                if (api_request_or_api_response[api_request_or_response_part][param_name][0] != "Array") and (
                        api_request_or_api_response[api_request_or_response_part][param_name][0] != "Property"):
                    consumers_or_producers[param_name] = \
                        api_request_or_api_response[api_request_or_response_part][param_name][0]
                else:
                    consumers_or_producers.update(
                        parse_param_struct(api_request_or_api_response[api_request_or_response_part][param_name]))
...
def solve_add_api_templates_json(add_api_templates_json):
    ...
    for add_api_template_dict in add_api_templates_json:
        if (len(list(add_api_template_dict["api_request"]["body"].keys())) == 1) and (
                add_api_template_dict["api_request"]["body"][
                    list(add_api_template_dict["api_request"]["body"].keys())[0]][0] == "MultipartParam"):
            # upload_api
            ...
            if (path_flag and (not param_flag)) or (param_flag):
                ...
                add_api_template = VoAPITemplate.ApiTemplate(add_api_template_dict["api_url"],
                                                             add_api_template_dict["api_method"],
                                                             add_api_template_dict["api_request"],
                                                             add_api_template_dict["api_response"])
    ...
    for candidate_api_seq_relation in candidate_api_seq_relations:
        ...
        if consumer_api == api_template:
            ...
            if consumer_param_value:
                update_api_request_param_value(api_template.api_request_value, consumer_param,
                                               [consumer_param_value, "VoAPI_CONSUMER"])
...
def write_every_candidate_api_test_log(log_file, candidate_api_seq, candidate_api_seq_relations,
    ...
    for candidate_api_seq_relation in candidate_api_seq_relations:
        log_str += "producer_api: {0} {1}\nproducer_param: {2}\nconsumer_api: {3} {4}\nconsumer_param: {5}\n------------\n".format(
            candidate_api_seq_relation["producer_api"].api_url, candidate_api_seq_relation["producer_api"].api_method,
            candidate_api_seq_relation["producer_param"], candidate_api_seq_relation["consumer_api"].api_url,
            candidate_api_seq_relation["consumer_api"].api_method, candidate_api_seq_relation["consumer_param"])
