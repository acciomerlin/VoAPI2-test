...
def get_sqlmap_path():
    ...
    if not os.path.exists(sqlmap_path):
        print('!!!!!!!!!!!\n' * 20)
        print('SQLMap not found, please install it using the command: pip install sqlmap')
...
ApiPathKeywords = {
    ...
    "upload_api": ["upload", "import", "file", "pic", "image", "img", "content", "page", "avatar", "attach", "submit",
                   "post"],
    "path_api": ["download", "export", "fetch", "file", "path", "category"],
    "command_api": ["set", "command", "cmd", "conf", "cfg", "rpc", "exec", "diagnose", "ping", "system", "ip",
                    "nslookup"],
    "database_api": ["sql", "database", "db", "query", "list", "search", "order", "select", "table", "column", "row"],
    "display_api": ["name", "content", "edit", "desc", "title", "view", "html", "link", "display", "code", "text",
                    "tab", "comment", "tag", "note"]
}

ApiParamKeywords = {
    "proxy_api": ["url", "uri", "host", "endpoint", "path", "href", "link", "proxy", "client", "remote", "fetch",
                  "dest", "redirect", "site", "callback", "hook", "img", "image", "access", "domain", "agent", "ping"],
...
ApiVulnerabilityPayloads = {
    ...
    "database_api": ["sqlmap"],
    "display_api": ["<img src='http://IP:PORT/xss{0}'>", "<img src='https://IP:PORT/xss{0}'>",
                    "<script>window.location='http://IP:PORT/xss{0}'</script>"]
}
...
RandomValueDict = {
    ...
    "Int": list(range(44)),
    "Bool": [True, False] * 22,
    "Object": [{"VoAPI" + str(i): False} for i in range(11)] + [{"VoAPI" + str(i): i} for i in range(11, 22)] + [
        {"VoAPI" + str(i): "VoAPI" + str(i - 11)} for i in range(22, 33)] + [
                  {"VoAPI" + str(i): [round(x, 2) for x in list(numpy.arange(4.40, 4.84, 0.01))][i]} for i in
                  range(33, 44)]
}
...
ProducerMethodPriority = {"POST": 4, "PUT": 3, "GET": 2, "PATCH": 1, "HEAD": 0, "DELETE": 0, "OPTIONS": 0, "TRACE": 0,
                          "CONNECT": 0}

ParamValuePriority = {"VoAPI_TEST": 7, "VoAPI_CONSUMER": 6, "VoAPI_PRODUCER": 6, "VoAPI_CUSTOM": 5,
                      "VoAPI_SPECIFICATION": 4, "VoAPI_FORMAT": 3, "VoAPI_SUCCESS": 2, "VoAPI_RANDOM": 1}

JellyfinBugUrls = ["/Videos/{itemId}/hls/{playlistId}/{segmentId}.{segmentContainer}",
                   "/Audio/{itemId}/hls1/{playlistId}/{segmentId}.{container}",
                   "/Videos/{itemId}/hls1/{playlistId}/{segmentId}.{container}", "/Audio/{itemId}/stream.{container}",
                   ...
                   "/LiveTv/LiveStreamFiles/{streamId}/stream.{container}", "/Videos/{itemId}/{stream}.{container}", ""]
